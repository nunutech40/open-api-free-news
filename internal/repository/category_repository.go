...
func (r *categoryRepository) FindAll(ctx context.Context) ([]*domain.Category, error) {
	...
	for rows.Next() {
		c := &domain.Category{}
		if err := rows.Scan(&c.ID, &c.Name, &c.Slug, &c.Description, &c.IsActive, &c.CreatedAt, &c.UpdatedAt); err != nil {
			return nil, err
		}
...
func (r *categoryRepository) FindBySlug(ctx context.Context, slug string) (*domain.Category, error) {
	query := `SELECT id, name, slug, description, is_active, created_at, updated_at FROM categories WHERE slug = $1`
	c := &domain.Category{}
...
func (r *categoryRepository) Create(ctx context.Context, c *domain.Category) (*domain.Category, error) {
	...
	err := r.db.QueryRowContext(ctx, query, c.Name, c.Slug, c.Description, now).Scan(
		&result.ID, &result.Name, &result.Slug, &result.Description, &result.IsActive, &result.CreatedAt, &result.UpdatedAt,
	)
	if err != nil {
		return nil, err
	}
